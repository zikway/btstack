#define BTSTACK_FILE__ "btstack_audio_ac695n.c"

/*
 *  btstack_audio_ac695n.c
 *
 *  Implementation of btstack_audio.h using hal_audio.h for ac695n run loop
 *
 */

#include "btstack_config.h"

// allow to compile all files in ac695n folder even if there's no audio support
...
static int btstack_audio_ac695n_sink_init(
    uint8_t channels,
    uint32_t samplerate,
...
static uint32_t btstack_audio_ac695n_sink_get_samplerate() {
    return BSP_AUDIO_OUT_GetFrequency();
}

static uint32_t source_samplerate = 0;

static int btstack_audio_ac695n_source_init(
    uint8_t channels,
    uint32_t samplerate,
...
static uint32_t btstack_audio_ac695n_source_get_samplerate() {
    return source_samplerate;
}

static void btstack_audio_ac695n_sink_set_volume(uint8_t volume){
    UNUSED(volume);
}

static void btstack_audio_ac695n_source_set_gain(uint8_t gain){
    UNUSED(gain);
}

static void btstack_audio_ac695n_sink_start_stream(void){
...
static void btstack_audio_ac695n_source_start_stream(void){
...
static void btstack_audio_ac695n_sink_stop_stream(void){
    // stop stream
    hal_audio_sink_stop();
    // stop timer
...
static void btstack_audio_ac695n_source_stop_stream(void){
    // stop stream
...
static void btstack_audio_ac695n_sink_close(void){
    // stop stream if needed
    if (sink_active){
        btstack_audio_ac695n_sink_stop_stream();
    }
    // close HAL
    hal_audio_sink_close();
}

static void btstack_audio_ac695n_source_close(void){
    // stop stream if needed
    if (source_active){
        btstack_audio_ac695n_source_stop_stream();
    }
    // close HAL
    hal_audio_source_close();
}

static const btstack_audio_sink_t btstack_audio_ac695n_sink = {
        .init           = &btstack_audio_ac695n_sink_init,
        .get_samplerate = &btstack_audio_ac695n_sink_get_samplerate,
        .set_volume     = &btstack_audio_ac695n_sink_set_volume,
        .start_stream   = &btstack_audio_ac695n_sink_start_stream,
        .stop_stream    = &btstack_audio_ac695n_sink_stop_stream,
        .close          = &btstack_audio_ac695n_sink_close
};

static const btstack_audio_source_t btstack_audio_ac695n_source = {
        .init           = &btstack_audio_ac695n_source_init,
        .get_samplerate = &btstack_audio_ac695n_source_get_samplerate,
        .set_gain       = &btstack_audio_ac695n_source_set_gain,
        .start_stream   = &btstack_audio_ac695n_source_start_stream,
        .stop_stream    = &btstack_audio_ac695n_source_stop_stream,
        .close          = &btstack_audio_ac695n_source_close
};

const btstack_audio_sink_t * btstack_audio_ac695n_sink_get_instance(void){
    return &btstack_audio_ac695n_sink;
}

const btstack_audio_source_t * btstack_audio_ac695n_source_get_instance(void){
    return &btstack_audio_ac695n_source;
}
